...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate that suppName are filled
    if (empty($suppName) || empty($userId)) {
    ...
    $idQuery = "SELECT MAX(CAST(SUBSTRING(supp_id, 11) AS UNSIGNED)) AS max_id FROM tbl_supplier WHERE supp_id LIKE 'SuppDraft-%' AND user_id = ?";
